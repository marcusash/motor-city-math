# Agent A (Application) — Training Document
...
You are **Agent A (Application Agent)** on Motor City Math. You build HTML, JS, CSS — the hands-on-keyboard implementation. You've shipped 42+ tasks, 19 HTML files, a 680-line shared engine, and a 313-question bank. You're productive and fast.
...
1. **High throughput** — you completed 42 of 44 assigned tasks in a single sprint
2. **Shared infrastructure** — `shared/styles.css` + `shared/scripts.js` + `shared/katex/` was well-architected
3. **Migration discipline** — all 13 legacy HTML files migrated to the shared CSS/JS system cleanly
4. **Design review response** — your response to the Association dispatch was thorough and showed you read the spec
5. **Postmortem ownership** — when called out, you took full responsibility and wrote concrete action items
...
**What happened:** You delegated `a-t4b-canvas-other` (5 legacy canvas files) to a sub-agent. That sub-agent also rewrote `nonlinear_exam_mvp.html` — a 70KB file it was NOT assigned. The result was a 3,024-line diff that:
...
Use **targeted string replacements only**. Find the exact lines, replace them. If you can't make a targeted edit, stop and ask FA.
...
# 4. Open in browser if possible — scroll top to bottom
```

### Rule 4: Review Gate on MVP Exam

**No changes to `nonlinear_exam_mvp.html` ship without FA sign-off.** After you make edits:
1. Commit with `--no-verify` if needed
2. Post in `.agent-status.md` what you changed and why
3. Wait for FA review before telling Marcus it's done
...
4. NEVER commit with message "File restored" — describe what was restored and why
...
- **The Answer Key modal is special** — it has `background: white` so it must use hardcoded dark colors, not CSS vars (because vars change in Arena Mode, white bg doesn't).
...
2. Check light mode
3. Toggle Arena Mode — check dark mode
...
### localStorage Keys
- `mcm_scores` — test results `{ storageKey: { score, total, pct, streak, timestamp } }`
- `mcm_srs` — spaced repetition `{ version: 1, questions: {}, sessions: [], streaks: {} }`
- `standardScores` — per-standard breakdown (used by parent.html)
- `mcm-arena-mode` — theme preference (`"on"` / `"off"`)

### File Sizes (Reference)
- `nonlinear_exam_mvp.html` — ~1493 lines, ~70KB (FRAGILE — see Rules 1-5)
- `shared/scripts.js` — ~680 lines
- `shared/styles.css` — ~1000+ lines
- `index.html` — ~550 lines
- `parent.html` — ~550 lines
...
```markdown
- [x] TASK-ID: Description — DONE ✅ (commit hash, what changed)
```

### When you hit a problem:
Post immediately. Don't try to "fix" things silently — that's how the regression happened.
```markdown
- [ ] TASK-ID: BLOCKED — {what's wrong, what you tried}
```

### When you need FA review:
```markdown
### 🔍 Ready for FA Review
- Files changed: `filename.html` (lines 100-120)
- What I did: {description}
- Verified: browser check ✅, line count ✅, single <body> ✅
```
...
- FA review gate active on MVP exam
...
1. **Shared CSS/JS extraction** — took 13 ad-hoc HTML files and unified them under one design system. Clean, no regressions.
2. **Practice mode (313 questions)** — SRS integration, random mode, unit filtering. Complex feature, shipped clean.
3. **Test builder** — teacher tool with print + answer key. Good use of the shared engine.
4. **Your PM-5 postmortem response** — honest, specific, actionable. That's the standard.

The worst:
1. **Both MVP file incidents** — in the same session, same file, same root cause. The dispatch warned you.
2. **Not verifying restorations** — "File restored" is not a verification strategy.
...
You're currently under full FA review. Here's the path to independence:

### Phase 1: Full Review (Current)
- FA reviews ALL your changes before they ship
- You must post a self-review in `.agent-status.md` before requesting FA review
...
- FA reviews only changes to: MVP exam, `shared/`, `index.html`, `parent.html`
...
### Phase 3: Spot-Check
- FA audits ~30% of your work, randomly selected
...
- FA does weekly audits
- Any critical finding sends you back to Phase 2

### What Moves You Forward
- Clean self-reviews that catch issues before FA does
...
### Review #1: Score Export/Import (Feb 18, 2026) — ✅ PASS
- **Spec compliance:** Exact match. Zero deviations.
- **Self-review quality:** Strong. Accurate reporting, honest about limitations (no browser testing from terminal), correctly flagged line count discrepancy.
- **Scope discipline:** Perfect — `git diff --stat` showed exactly 2 implementation files.
- **Graduation progress:** 1 of 5 toward Phase 2.
- **Spec bug found by FA (not Agent A's fault):** standardScores merge is additive — re-importing same file doubles standard scores. Timestamp dedup protects mcm_scores but not standardScores. Follow-up needed.
- **What to keep doing:** Honest self-reviews that flag what you can't verify. Following specs exactly.

---

## 12. Current Assignment

Read `.agent-a-score-export-dispatch.md` for your next task. It's a training exercise — FA will evaluate your work against the implementation spec in `.score-export-import-spec.md`.

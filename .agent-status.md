# Motor City Math — Agent Status Board

> Live task tracking for all agents. Each agent owns their section.
> Update after every completed task. Use `- [x]` for done, `- [ ]` for in progress.
> Read `.agents.md` for roles. Read `.agent-protocol.md` for rules.

---

## Application Agent (A) Status
_Domains: Build system, shared components, templating, dashboard, grading engine_

> **Executive Summary:** Not yet started. Waiting for agent onboarding.

### Backlog
- [ ] A-1: Extract shared CSS from 12 HTML files → `shared/styles.css`
- [ ] A-2: Extract shared JS from 12 HTML files → `shared/scripts.js`
- [ ] A-3: Create Chart.js theme file → `shared/chart-theme.js`
- [ ] A-4: Create print stylesheet → `shared/print.css`
- [ ] A-5: Migrate all 12 HTML files to use shared CSS/JS (one at a time, verify each)
- [ ] A-6: Build auto-grading engine (extend `check()` to all question types)
- [ ] A-7: Add auto-grading to the 10 files that lack it
- [ ] A-8: Build dashboard (`index.html`) — test index, scores, completion status
- [ ] A-9: Implement KaTeX rendering across all tests (replace MathJax in 4 files)
- [ ] A-10: Per-standard scoring display in footer
- [ ] A-11: Streak counter (consecutive correct answers)
- [ ] A-12: Build question renderer from JSON bank
- [ ] A-13: Build randomized practice mode
- [ ] A-14: Build test builder (pick standards + difficulty → generate test)

### Cross-Agent Requests
_(none yet)_

---

## Research Agent (R) Status
_Domains: Curriculum alignment, question accuracy, pedagogy, question bank_

> **Executive Summary:** Not yet started. Waiting for agent onboarding.

### Backlog
- [ ] R-1: Audit all ~200 existing questions for mathematical accuracy
- [ ] R-2: Verify all answer keys are correct (every answer in all 12 files)
- [ ] R-3: Tag each question with: standard, difficulty (1-5), question type (MC/short/graph/proof)
- [ ] R-4: Map questions to SAAS curriculum standards (Unit 1-4)
- [ ] R-5: Build `data/questions.json` schema + initial population from existing tests
- [ ] R-6: Build `data/standards.json` — all Algebra II standards with descriptions
- [ ] R-7: Write step-by-step solutions for all questions
- [ ] R-8: Write hints for all questions (scaffolded: hint 1, hint 2, then answer)
- [ ] R-9: Design spaced repetition algorithm (missed Qs return at 1d, 3d, 7d intervals)
- [ ] R-10: Research difficulty calibration — are the questions appropriately hard for Algebra II?
- [ ] R-11: Parse `Winter Tri Standards Worksheets/` PDF into structured question data
- [ ] R-12: Generate 50 new questions per unit to expand the question bank

### Cross-Agent Requests
_(none yet)_

---

## Design Agent (D) Status
_Domains: Visual design, UX, interaction patterns, responsive, accessibility_

> **Executive Summary:** 4 major deliverables complete. Design system, voice guide, agent onboarding, and R-agent message all shipped.

- [x] D-1: Project inventory and review — DONE ✅ `.project-review.md`
- [x] D-2: Design system v2.0 — DONE ✅ `.design-system.md` (15 sections, Pistons palette, components, typography, layout, dark mode, ADHD-aware voice, SAAS school context)
- [x] D-2b: Name brainstorm — DONE ✅ "Kai's Study Lab" → **"Motor City Math"** (Pistons + math)
- [x] D-13: Voice guide — DONE ✅ `.voice-guide.md` (Marcus voice patterns, copy templates for all states, ADHD guidelines, Agent R placeholder)
- [x] D-14: Agent onboarding — DONE ✅ `.agent-onboarding.md` (master doc for all 6 agents, merged with Application agent section)
- [x] D-15: Research agent onboarding message — DONE ✅ `.agent-r-onboarding-message.md` (5 tasks, JSON schema, Kai context)

### Backlog
- [ ] D-3: Dashboard wireframe and spec
- [ ] D-4: Score card component spec (passing/needs-work states)
- [ ] D-5: Progress chart specs (Chart.js configs for score history, standard mastery)
- [ ] D-6: Mobile responsive implementation spec (breakpoints, touch targets)
- [ ] D-7: Dark mode ("Arena Mode") CSS variables spec
- [ ] D-8: Streak counter UI spec (animation, placement, visual treatment)
- [ ] D-9: Question type component specs (MC, short answer, graphing, proof)
- [ ] D-10: Hint system UI (progressive reveal, animation)
- [ ] D-11: Print layout audit + spec (header with name/date fields)
- [ ] D-12: Accessibility audit (WCAG AA compliance, keyboard nav, screen reader)

### Cross-Agent Requests
_(none yet)_

---

## Fundamentals Agent (F) Status
_Domains: Testing, validation, grading accuracy, edge cases, print QA_

> **Executive Summary:** Not yet started. Waiting for agent onboarding.

### Backlog
- [ ] F-1: Audit auto-grading logic in `index.html` and `index_calc.html` — verify tolerance, edge cases
- [ ] F-2: Write validation tests for answer checking (numeric tolerance, string comparison, LaTeX equivalence)
- [ ] F-3: Test print layout on all 12 files (Chrome, Safari, Firefox)
- [ ] F-4: Test save/load (localStorage) round-trip on all 7 files that support it
- [ ] F-5: Test Chart.js rendering on all 7 files with charts
- [ ] F-6: Test mobile layout at 375px, 768px, 1024px breakpoints
- [ ] F-7: Write regression tests for grading engine (prevent wrong answers from being accepted)
- [ ] F-8: Test KaTeX rendering after migration (all equations display correctly)
- [ ] F-9: Cross-browser testing (Chrome, Safari, Firefox, Edge)
- [ ] F-10: Question bank JSON schema validation tests
- [ ] F-11: Dashboard data integrity tests (scores match actual test results)
- [ ] F-12: Test offline mode — CDN failures should not break the app

### Cross-Agent Requests
_(none yet)_

---

## Security Agent (S) Status
_Domains: Data integrity, progress tracking, offline-first, privacy_

> **Executive Summary:** Not yet started. Waiting for agent onboarding.

### Backlog
- [ ] S-1: Design localStorage schema for progress tracking (per-test scores, per-question history, timestamps)
- [ ] S-2: Audit existing save/load code for data corruption risks
- [ ] S-3: Design data export format (JSON backup of all progress)
- [ ] S-4: Ensure no student PII beyond first name in any file
- [ ] S-5: Offline-first audit — what breaks without internet? (CDN deps: KaTeX, Chart.js)
- [ ] S-6: Design migration path if localStorage schema changes
- [ ] S-7: Test data persistence across browser updates, cache clears
- [ ] S-8: Evaluate IndexedDB vs localStorage for question bank storage

### Cross-Agent Requests
_(none yet)_

---

## Platform Agent (P) Status
_Domains: Agent coordination, CI/CD, docs, repo hygiene, taskboards_

> **Executive Summary:** Not yet started. Waiting for agent onboarding.

### Backlog
- [ ] P-1: Set up repo structure (shared/, tests/, data/, docs/ directories)
- [ ] P-2: Write README.md (project description, how to use, how to contribute)
- [ ] P-3: Set up .gitignore (node_modules if added, .DS_Store, etc.)
- [ ] P-4: File naming convention migration (consistent kebab-case)
- [ ] P-5: Create `.marcus-queue.md` template
- [ ] P-6: Agent onboarding verification (all agents have read protocol + design system)
- [ ] P-7: Define commit message conventions
- [ ] P-8: Create taskboard refresh schedule

### Cross-Agent Requests
_(none yet)_

---

## Bugs

| ID | Severity | Description | File | Owner | Status |
|----|----------|-------------|------|-------|--------|
| _(none filed yet)_ | | | | | |

---

*Last updated: 2026-02-17 by application-agent (initial setup)*

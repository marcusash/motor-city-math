...
/* === Spaced Repetition System (Modified Leitner) === */

var MCM_SRS = (function() {
    var SRS_KEY = 'mcm_srs';
    var INTERVALS = [0, 0, 1, 3, 7, 14]; // box → days until next review

    function load() {
        try {
            var data = JSON.parse(localStorage.getItem(SRS_KEY));
            if (data && data.version === 1) return data;
        } catch(e) {}
        return { version: 1, questions: {}, sessions: [], streaks: { current: 0, best: 0, last_date: '' } };
    }
    ...
    function recordAnswer(qId, isCorrect) {
        var data = load();
        if (!data.questions[qId]) data.questions[qId] = { box: 0, last: '', correct: 0, wrong: 0 };
        var q = data.questions[qId];
        q.last = today();
        if (isCorrect) { q.correct++; q.box = Math.min(q.box + 1, 5); }
        else { q.wrong++; q.box = 1; }
        save(data);
    }

    function recordSession(count, correct, timeSec) {
        var data = load();
        var t = today();
        data.sessions.push({ date: t, count: count, correct: correct, time_sec: timeSec || 0 });
        if (data.sessions.length > 100) data.sessions = data.sessions.slice(-100);
        var lastDate = data.streaks.last_date;
        if (lastDate !== t) {
            data.streaks.current = daysSince(lastDate) === 1 ? data.streaks.current + 1 : 1;
        }
        data.streaks.last_date = t;
        if (data.streaks.current > data.streaks.best) data.streaks.best = data.streaks.current;
        save(data);
    }

    function buildQueue(questionBank, filters, maxSize) {
        maxSize = maxSize || 15;
        filters = filters || {};
        var data = load();
        var lastSession = data.sessions.length > 0 ? data.sessions[data.sessions.length - 1] : null;
        var daysSinceSession = lastSession ? daysSince(lastSession.date) : 999;

        var pool = questionBank.filter(function(q) {
            if (filters.unit && q.unit !== filters.unit) return false;
            if (filters.standard && q.standard !== filters.standard) return false;
            if (filters.difficulty && q.difficulty !== filters.difficulty) return false;
            return true;
        });

        var annotated = pool.map(function(q) {
            var srs = data.questions[q.id] || { box: 0, last: '', correct: 0, wrong: 0 };
            var isDue = srs.box <= 1 || daysSince(srs.last) >= INTERVALS[srs.box];
            return { q: q, box: srs.box, last: srs.last, isDue: isDue };
        });

        // ADHD: if returning after 3+ days, prioritize easy wins (high box) first
        if (daysSinceSession > 3) {
            annotated.sort(function(a, b) {
                if (a.isDue !== b.isDue) return a.isDue ? -1 : 1;
                return b.box - a.box || ((a.last||'') < (b.last||'') ? -1 : 1);
            });
        } else {
            annotated.sort(function(a, b) {
                if (a.isDue !== b.isDue) return a.isDue ? -1 : 1;
                return a.box - b.box || ((a.last||'') < (b.last||'') ? -1 : 1);
            });
        }

        var due = annotated.filter(function(a) { return a.isDue; });
        var notDue = annotated.filter(function(a) { return !a.isDue; });
        var selected = due.slice(0, maxSize);
        if (selected.length < maxSize) selected = selected.concat(notDue.slice(0, maxSize - selected.length));
        return selected.map(function(a) { return a.q; });
    }

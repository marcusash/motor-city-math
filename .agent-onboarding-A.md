...
> **You are Agent A (Application) for Motor City Math.**
...
**Motor City Math** is a collection of 12 hand-built HTML practice tests for Algebra II. Marcus (your human lead, CVP of Design at Microsoft) built these with AI assistance to help his 15-year-old son Kai prepare for exams at Seattle Academy of Arts & Sciences (SAAS). **Result: Kai went from struggling to earning an A.**

**School context:** SAAS uses standards-based grading. Cardinals mascot, red/black/white school colors. The tests already reference SAAS standards (3.a, 3.b, etc.). Mission: Question, Imagine, Create, Contribute.

**Kai context:** Kai is 15 and has ADHD. This means: copy must be clear, short, and motivating — never wordy. UI should be scannable, not dense. Feedback should be immediate and direct. Think coach at halftime, not textbook.

**Brand:** Detroit Pistons-inspired (Kai's team). Cade Cunningham's breakout All-Star season parallels Kai's grade turnaround — from struggling to dominant. See `.design-system.md` for the full Pistons palette and `.voice-guide.md` for copy guidelines.
...
1. **Read all project files:** `.design-system.md`, `.project-review.md`, `.voice-guide.md` (if exists), `.agent-protocol.md`, `.agents.md`

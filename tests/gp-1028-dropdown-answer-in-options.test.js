// gp-1028-dropdown-answer-in-options.test.js — dropdown answer must be one of the option values
...
for (const file of RP_FILES) {
  ...
  for (const q of data.questions) {
    for (const inp of (q.inputs || [])) {
      ...
      const opts = (inp.options || []).map(o => o.value || o);
...
console.log(`gp-1028-dropdown-answer-in-options: ${pass} pass, ${fail} fail`);

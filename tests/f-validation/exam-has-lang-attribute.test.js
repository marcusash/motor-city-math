...
console.log('\u{1F3C0} exam-has-lang-attribute.test.js\n');

var html = fs.readFileSync(path.join(__dirname, '../../exam.html'), 'utf-8');

var hasLang = /<html[^>]+lang\s*=\s*["']en/.test(html);

test('exam.html <html> has lang="en" attribute', hasLang);

console.log('\n' + '='.repeat(50));
console.log('exam-has-lang-attribute: ' + (pass+fail) + ' checks, ' + pass + ' pass, ' + fail + ' fail');

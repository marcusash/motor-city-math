// gp-1237-radio-inputs-min-2-options.test.js
...
for (const file of RP_FILES) {
  ...
  for (const q of data.questions) {
    for (const inp of (q.inputs || []).filter(i => i.type === 'radio')) {
      ...
      if (n >= 2) pass++;
      else { fail++; failures.push(file + ': ' + q.id + '.' + inp.id + ' has ' + n + ' option(s)'); }
    }
  }
}
console.log('gp-1237-radio-min-2-options: ' + pass + ' pass, ' + fail + ' fail');

...
// Section A type sequence for all exams: RP1-7/RP12 = [identify,multiple_choice,multiple_choice]
// RP8-11 = [quadratic,multiple_choice,multiple_choice]
...
for (const file of RP_FILES) {
  ...
  const isOlder = OLDER.has(id);
  const isNewer = NEWER.has(id);
  const expQ1 = isOlder ? 'identify' : (isNewer ? 'quadratic' : null);

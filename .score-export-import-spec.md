# Score Export/Import — Implementation Spec for Agent A

> Written by: Agent A2 (Staff Engineer)
> Date: Feb 18, 2026
> Status: Ready for implementation
> Review: Agent A must submit implementation summary + code diff for A2 review before merging
...
1. **Export button** on Kai's dashboard (`index.html`) — downloads a JSON file
2. **Import button** on Marcus's parent dashboard (`parent.html`) — reads that JSON and loads the data
...
A "📤 Send Scores to Dad" button that downloads all MCM data as a JSON file.
...
<div style="text-align:center; margin: 16px 0 8px;">
    <button class="nav-btn secondary" onclick="exportScores()">📤 Send Scores to Dad</button>
</div>
```
...
- [ ] No external dependencies — pure JS Blob API
...
A "📥 Import Kai's Scores" button that reads an exported JSON file and merges data into `localStorage`.

### Placement

Add between the "← Back to Dashboard" link and the footer (around line 184-186):

```html
<div style="text-align:center; margin: 16px 0 8px;">
    <button class="nav-btn secondary" onclick="document.getElementById('importFile').click()">📥 Import Kai's Scores</button>
...
function importScores(input) {
    ...
    reader.onload = function(e) {
        try {
            ...
            document.getElementById('importStatus').textContent = '❌ Invalid file — not valid JSON.';
            return;
        }

        // Validate structure
        if (!data.mcm_scores && !data.mcm_srs) {
            document.getElementById('importStatus').textContent = '❌ No score data found in this file.';
            return;
        }
        ...
        // Merge standardScores (additive — sum correct/total per standard)
        ...
        document.getElementById('importStatus').textContent =
            '✅ Imported ' + imported + ' test score' + (imported !== 1 ? 's' : '') + ' from ' + dateStr;
...
- [ ] Validates JSON structure — shows error if invalid or missing data
- [ ] **Smart merge for mcm_scores**: per test key, keeps the entry with the latest `timestamp`
- [ ] **Additive merge for standardScores**: sums correct/total per standard
...
The footer on parent.html uses `color: var(--border-default)` — same bug we fixed on index.html. Change it:
...
### Pre-flight
- [ ] Read `index.html` lines 190-200 (placement area) — note current line count
- [ ] Read `parent.html` lines 180-190 (placement area) — note current line count
- [ ] Run `git status` — confirm clean working tree

### After implementation
- [ ] `git diff --stat` — only `index.html` and `parent.html` should show changes
- [ ] Line counts: `index.html` increased by ~25 lines, `parent.html` increased by ~55 lines
- [ ] Open `index.html` in Chrome — verify export button visible
- [ ] Toggle Arena Mode — verify button visible in dark mode
- [ ] Click "Send Scores to Dad" with no scores — should show alert
- [ ] Take a quick test, grade it, go back to dashboard, click export — should download JSON
- [ ] Open the JSON file — verify it has `version`, `exported`, `mcm_scores` keys
- [ ] Open `parent.html` — verify import button visible
- [ ] Toggle Arena Mode on parent.html — verify button visible in dark mode
- [ ] Click "Import Kai's Scores" — select the exported JSON file
- [ ] Verify success message appears with correct count and date
- [ ] Verify page reloads and charts show the imported data
- [ ] Try importing the same file again — should NOT create duplicate scores
- [ ] Try importing a non-JSON file — should show error message

### Self-review (write this up before requesting A2 review)
...
This task is a training exercise. A2 will evaluate:
...
3. **Did you write a self-review?** Quality matters — did you catch issues before A2 did?
...
6. **Is the merge logic correct?** This is the hardest part — think about edge cases.

---

_After completing implementation, post your full self-review and `git diff` in `.agent-status.md` under your section. Tag it "Ready for A2 Review." Do NOT tell Marcus it's done until A2 signs off._

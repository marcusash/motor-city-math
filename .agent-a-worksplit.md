# Agent A Work Split — Application Agent Team
...
| Role | Agent | Focus |
|------|-------|-------|
| **Staff Engineer (A2)** | Incoming senior agent | Architecture, MVP exam build, dashboard, complex features |
...
- [x] **A-1:** Shared CSS → `shared/styles.css` (13KB Pistons design system)
- [x] **A-2:** Shared JS → `shared/scripts.js` (gradeTest engine, answer key, save/load, print)
- [x] **A-3:** Chart.js theme → `shared/chart-theme.js`
- [x] **A-4:** Print stylesheet → `shared/print.css`
...
- **Agent F:** Built 3 test suites for MVP (graphing e2e, grading, score storage — 182 tests)
...
**Spec:** `.mvp-spec.md` — read the whole thing.  
**Hints:** `data/mvp-exam-hints.json` — Agent R's verified hints for all 15 questions.  
**Tests:** `tests/f-validation/mvp-*.test.js` — 3 test suites ready.
...
3. Sticky notation bar, parent function dropdowns, ± labeled inputs, radio buttons for Q14
...
6. SAAS 4-point grade display (≥92=4, ≥82=3, ≥70=2, <70=1)
...
| Q4 | x = 2 | single input |
| Q5 | x = 0.5, x = -1.5 | 2 inputs (±) |
| Q6 | x = ±2.236 (±√5) | 2 inputs (±) |
| Q7 | x = 2 | single input |
| Q8 | x = 5.667 (17/3) | single input |
| Q9 | x = 3 | single input |
| Q10 | x = ±2.236 (±√5) | 2 inputs (±) |
| Q11 | x = 9 | single input |
| Q12 | graph: f(x) = -(x+2)²+5, vertex (-2,5) | canvas |
| Q13 | graph: f(x) = 3/(x-1)-2, VA x=1, HA y=-2 | canvas + inputs |
| Q14 | option B: r = √(A/π) | radio buttons |
...
### Staff Engineer (A2) — Senior/Architecture

| Task | Description | Priority |
|------|-------------|----------|
| **MVP Build** | Build `nonlinear_exam_mvp.html` per `.mvp-spec.md` | 🔴 URGENT |
...
### Agent A (Original) — Mechanical/Migration

| Task | Description | Priority |
|------|-------------|----------|
| **Bug: index.html grading** | Answers object is copy-pasted from index_calc.html — all wrong. Fix using `data/questions.json` as source of truth | High |
| **Bug: index_calc.html Q16b** | 28000×0.85⁴ = $14,616.18 not $14,636.56 | High |
...
| **Norm() improvements** | Handle superscript chars (x¹¹ vs x^11), dollar signs, commas in parseFloat | Medium |
...
```
shared/
  styles.css      — 13KB Pistons design system (all files link this)
  scripts.js      — gradeTest(), showAnswerKey(), saveResults(), loadResults(), etc.
  chart-theme.js  — Chart.js color defaults
  print.css       — @media print rules

data/
  questions.json         — Agent R's verified question bank (142 questions)
  mvp-exam-hints.json    — Hints for MVP exam (Agent R)

tests/
  f-validation/
    mvp-grading.test.js       — 201 lines
    mvp-graphing-e2e.test.js  — 252 lines  
    mvp-score-storage.test.js — 328 lines
```
...
gradeTest({
  ...
  feedbacks: [
    ['✅ Correct!', '❌ Review this one.'],  // [correct msg, incorrect msg]
  ],
...
- One CTA at a time  
- Celebrate results not effort ("🔥 Locked in." not "Good try!")

### Git rules:
- NEVER run `git config` — repo uses Marcus's global identity
...
- **Status board:** `.agent-status.md` — update after every completed task
- **File ownership:** `.agents.md` — check before editing any file

# 🏀 MCM Autonomous Sprint — Paste This Into Every Agent Window

**From Marcus (via FA). I'm away for 2 hours. No one can unblock you. Read this carefully.**

---

## MISSION: Ship exam.html renderer + retake exam tonight

Kai's retake is Tue–Thu. Every hour without a working practice exam costs a practice cycle. This is the priority above all else.

## THE PLAN

```
Phase 1: Agent GA builds exam.html          ← THE CRITICAL PATH
Phase 2: Agent GF tests it                  ← Validates grading accuracy
Phase 3: Agent GR verifies math             ← Confirms all 15 answers grade correctly
...
### Agent GA (Application) — YOU ARE THE CRITICAL PATH
...
**Build order:**
1. `git pull` — get FA's answers and Agent R's retake JSON
2. Read `docs/exam-renderer-spec.md` (the full spec)
3. Read `data/retake-practice-1.json` (the test data — 15 questions)
4. Build `exam.html` — JSON-driven renderer that reads `?file=` URL param
...
7. Canvas graphing: extract the pattern from `nonlinear_exam_mvp.html` — drawGrid, click-to-plot, checkGraph, tooltip
...
**If you get blocked:**
- Canvas code is complex → look at `nonlinear_exam_mvp.html` lines with `drawGrid`, `redrawGraph`, `checkGraph`. Extract the pattern, don't copy the whole file.
...
- KaTeX rendering: questions use `\( ... \)` LaTeX — include the KaTeX CDN like other test files do.

**When done:** Commit with message `feat(GA): exam.html JSON renderer — 15 questions grading`. Push. Then update your section in `.agent-status.md` saying "exam.html shipped, ready for GF testing."

---

### Agent GR (Research) — MATH VERIFICATION
...
6. Verify hint → answer → solution_steps layers work on wrong answers
...
- Review `data/retake-practice-1.json` one more time — triple-check every answer, every tolerance, every solution step
- Write `data/retake-practice-2.json` — a second practice exam for Kai (different numbers, same standards). Kai will need more than one practice run.
...
### Agent GF (Fundamentals) — TEST VALIDATION

**Wait for GA to push exam.html, then:**
1. `git pull`
2. Test all 15 questions — correct answers should score 15/15
...
7. Test scorecard: verify SAAS grading (≥92=4, ≥82=3, ≥70=2, <70=1)
...
**If GA hasn't pushed yet:** Don't just wait. Do this instead:
- Write Playwright test stubs for exam.html — you know the spec, you can write the test structure now
- Review existing test suite — run `npx playwright test` and report any regressions
- Validate `data/retake-practice-1.json` schema — write a Node.js script that checks every question has required fields (id, number, standard, inputs, hint, solution_steps, feedback_correct, feedback_wrong)

**When done:** Commit test results or test scripts to the repo. Update `.agent-status.md`.

---

### Agent GP (Platform), GD (Design), GS (Security), GI (Data) — SUPPORT
...
**GD:** After GA pushes exam.html, do a quick visual QA — does it match the Pistons design system? Arena Mode toggle? Mobile responsive at 375px? Post findings in `.agent-status.md`.

**GS:** After GA pushes, verify localStorage behavior — does `mcm_scores` save correctly? Does import/export still work? No key collisions?
...
✅ `exam.html?file=retake-practice-1` renders 15 questions in the browser
✅ Answering correctly scores 15/15 with SAAS grade
✅ Wrong answers show hint → answer → solution_steps
✅ Per-standard breakdown appears in scorecard
✅ Timer counts down from 60 minutes
✅ Scores save to localStorage (`mcm_scores`)
✅ Graph questions (Q12, Q13) have interactive canvas with click-to-plot
✅ Plus/minus questions accept either ordering
...
✅ 13 non-graph questions rendering and grading correctly
✅ Q12/Q13 show as "graph question — interactive grading coming soon"
This still gives Kai 13 practice questions to work with tomorrow.

## TIMELINE

```
Now → +45 min:  GA builds exam.html (non-graph questions first)
+45 min → +60:  GA adds graph support (or ships without and iterates)
+60 → +75 min:  GA pushes. GR + GF pull and start testing.
+75 → +90 min:  GR verifies math. GF tests edge cases.
+90 → +120:     Fix any bugs found. Final push.
Marcus returns:  Everything is green.
```

## ⚠️ KEEP-ALIVE PROTOCOL — NO IDLE AGENTS

**This is critical. Marcus cannot unblock you. You must keep yourself busy.**

### Every agent, every 15 minutes:
1. `git pull` — check for new commits from other agents
2. Read `.agent-status.md` — look for messages, blockers, requests directed at you
3. Read `.autonomous-sprint.md` — re-read your section if you're unsure what to do next
...
- **GP:** Audit the dashboard — does `index.html` link to exam.html? Should it? Draft the link/navigation update for GA.
...
2. Try to work around it yourself — choose the simpler path
...
### 🔴 THE GOLDEN RULE: If you have nothing to do, you haven't looked hard enough.
